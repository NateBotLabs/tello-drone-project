...
def stream_drone(me):
    try:
        while True:
            # Capture a video frame from the drone's camera
            img = me.get_frame_read().frame

            # Resize the video frame to a smaller size (360x240)
            img = cv2.resize(img, (360, 240))

            # Display the video frame in a window titled "Drone Video Streaming"
            cv2.imshow('Drone Video Streaming', img)

            # Wait for a key event with a timeout of 1 millisecond
            cv2.waitKey(1)
    except KeyboardInterrupt:
        # Catch a KeyboardInterrupt exception when the user interrupts the program
        exit(1)
    finally:
        # Print a message indicating that the streaming has ended
